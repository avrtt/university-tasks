     ...
     & mem(3 * N + 2 * nodes + 6))
        ...
        CALL matrixNew(mem(1), mem(N + 2), mem(2*N+3),
...
! MAIN FUNCTIONS
    ...
    1   PRINT *, 'vector.txt does not exist!'
      ...
      SUBROUTINE matrixNew(iA, Di, Al, Au, A)
        COMMON /size/ N, nodes 
        
        DIMENSION iA(*), Di(*), Al(*), Au(*), A(N, N)
       
        OPEN(1, FILE = 'matrix_new.txt')
        DO i = 1, N
          A(i, i) = Di(i)
        END DO
...
! CONVERSION FUNCTIONS
      ...
      SUBROUTINE toDirect(iA, Di, Al, Au, V)
        COMMON /size/ N, nodes 
        DIMENSION iA(*), Di(*), Al(*), Au(*)
        
        CALL matrixToDir(iA, Di, Al, Au)
        CALL vectorToDir(V)
      END
      ...
      SUBROUTINE matrixToDir(iA, Di, Al, Au)
        COMMON /size/ N, nodes 
